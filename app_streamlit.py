...
# Titre Principal
st.markdown(f'<h1 class="svg-align">{SVG_ICONS["chart"]} Plateforme MLOps : Prédiction & Analyse</h1>', unsafe_allow_html=True)
st.caption("Suivi des métriques clés et déploiement de modèles en temps réel")

# --- SIDEBAR ---
st.sidebar.markdown(f'<div class="svg-align">{SVG_ICONS["folder"]} <b>Source des données</b></div>', unsafe_allow_html=True)
csv_url = st.sidebar.text_input("URL du fichier CSV (Optionnel)", placeholder="https://url-du-fichier.csv", label_visibility="collapsed")
...
try:
    ...
    st.sidebar.markdown(f'<div class="svg-align" style="color:#10B981;">{SVG_ICONS["success"]} Données connectées</div>', unsafe_allow_html=True)
except Exception as e:
    st.sidebar.markdown(f'<div class="svg-align" style="color:#EF4444;">{SVG_ICONS["error"]} Erreur : {e}</div>', unsafe_allow_html=True)
    try:
...
st.markdown(f'<h3 class="svg-align">{SVG_ICONS["prediction"]} Simulation d\'Inférence Client</h3>', unsafe_allow_html=True)
with st.container(border=True):
    ...
    if st.button("Calculer le Score de Risque", type="primary"):
        client_payload = {
            "CreditScore": cs, "Gender": gender_encoded, "Age": age, "Tenure": tenure, 
            "Balance": balance, "NumOfProducts": num_prod, "HasCrCard": has_card_encoded, 
...
with tab1:
    if not df.empty:
        ...
        c1, c2 = st.columns(2)
        with c1:
            feature = st.selectbox("Distribution de la variable :", ["Age", "CreditScore", "Balance", "EstimatedSalary"])
        ...
        st.warning("Veuillez connecter une source de données valide pour afficher les visualisations.")
            ...
            if st.button("Exécuter /prepare"):
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        st.error(f"Échec de la préparation (Statut : {res.status_code})")
                except Exception as e:
                    st.error(f"Impossible de joindre l'API : {e}")
                
    with col_btn2:
        with st.container(border=True):
            st.markdown(f'<div class="svg-align">{SVG_ICONS["cpu"]} <b>Étape 2 : Entraînement</b></div>', unsafe_allow_html=True)
            chosen_model = st.selectbox("Modèle cible :", ["random_forest", "logistic_regression", "gradient_boosting"])
            ...
            if st.session_state.trained_model:
                if isinstance(st.session_state.trained_model, dict):
                    ...
                    try:
                        ...
                        st.error(f"Fichier de modèle '{filename}' introuvable localement.")
                elif st.session_state.trained_model == "error":
                    st.markdown(f'<div class="svg-align" style="color:#EF4444; margin-top:10px;">{SVG_ICONS["error"]} Erreur d\'entraînement.</div>', unsafe_allow_html=True)
                else:
                    err_msg = st.session_state.trained_model.replace("connection_error: ", "")
                    st.markdown(f'<div class="svg-align" style="color:#EF4444; margin-top:10px;">{SVG_ICONS["error"]} Impossible de joindre l\'API : {err_msg}</div>', unsafe_allow_html=True)